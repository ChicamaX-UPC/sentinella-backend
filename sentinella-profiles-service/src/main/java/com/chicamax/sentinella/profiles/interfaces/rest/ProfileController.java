...
public class ProfileController {
    ...
    @GetMapping("/me")
    public ResponseEntity<ProfileResource> getMe(@AuthenticationPrincipal Jwt jwt) {
        UUID userId = UUID.fromString(jwt.getSubject());
        var profile = profileCommandService.findOrCreateForUser(userId);
        return ResponseEntity.ok(profileResourceAssembler.toResource(profile));
    }

    @PatchMapping("/me")
    public ResponseEntity<ProfileResource> patchMe(
            @AuthenticationPrincipal Jwt jwt,
            @Valid @RequestBody PatchProfileResource resource
    ) {
        UUID userId = UUID.fromString(jwt.getSubject());
