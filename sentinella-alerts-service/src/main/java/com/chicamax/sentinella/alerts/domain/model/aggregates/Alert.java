...
public class Alert extends AuditableAbstractAggregateRoot<Alert> {
    ...
    public void acknowledge(UUID actorId, OffsetDateTime when) {
        if (status != AlertStatus.RECEIVED) {
            throw new ResponseStatusException(HttpStatus.CONFLICT, "La alerta solo puede reconocerse desde RECEIVED");
        }
    ...
    public void complete(UUID assigneeId) {
        if (status != AlertStatus.ACKNOWLEDGED) {
            throw new ResponseStatusException(HttpStatus.CONFLICT, "La alerta solo puede completarse desde ACKNOWLEDGED");
        }
    ...
    public void close(UUID actorId, OffsetDateTime when, String notes) {
        if (status != AlertStatus.COMPLETED) {
            throw new ResponseStatusException(HttpStatus.CONFLICT, "La alerta solo puede cerrarse desde COMPLETED");
        }
